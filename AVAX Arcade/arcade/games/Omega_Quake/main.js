...
async function main() {
	...
	try {
		...
		// Network choice modal - Connect to AVAX Fuji
		const connectFuji = document.getElementById('connect-fuji');
		if (connectFuji) {
			connectFuji.addEventListener('click', async (e) => {
				e.stopPropagation();
				networkChoiceModal.style.display = 'none';
				walletManager.setSelectedNetwork('fuji');
