...
// AVAX Fuji Arcade Leaderboard (gameId 1 = Arcade Bot)
...
// Deploy leaderboard to Fuji and set address in DEPLOY_LEADERBOARDS_FUJI.md
...
class ContractManager {
    ...
    private constructor() {
        this.contractAddress = localStorage.getItem('avax_fuji_leaderboard_address') || DEFAULT_CONTRACT_ADDRESS;
    }
    ...
    public setContractAddress(address: string): void {
        ...
        localStorage.setItem('avax_fuji_leaderboard_address', address);
    }
    ...
    /**
     * Submit score on-chain to Avalanche Fuji
     */
    public async submitScoreOnChain(score: number): Promise<string> {
        ...
        if (!state.isCorrectNetwork) {
            await walletManager.switchToFuji();
        }
