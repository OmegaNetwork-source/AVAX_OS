// WalletManager.ts - Handles MetaMask wallet connection for Avalanche Fuji Testnet

// Avalanche Fuji C-Chain Configuration
export const AVAX_FUJI_NETWORK = {
...
class WalletManager {
  ...
  public async switchToFuji(): Promise<boolean> {
    if (!window.ethereum) return false;

    try {
      await window.ethereum.request({
        method: 'wallet_switchEthereumChain',
        params: [{ chainId: AVAX_FUJI_NETWORK.chainId }]
      });
  ...
  private async checkNetwork(): Promise<void> {
    ...
    try {
      ...
      const isCorrect = chainId === AVAX_FUJI_NETWORK.chainId;
