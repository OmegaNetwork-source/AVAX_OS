...
// Configure wallet for identity registration (AVAX Fuji + Hot Wallet Mode)
...
function setupWindowControls() {
    const backBtn = document.getElementById('backToDesktopBtn');
    const minimizeBtn = document.getElementById('minimizeBtn');
    const maximizeBtn = document.getElementById('maximizeBtn');
    const closeBtn = document.getElementById('closeBtn');

    if (window.OMEGA_WEB_MODE) {
    ...
    closeBtn?.addEventListener('click', () => {
        if (window.OMEGA_WEB_MODE && window.electronAPI && window.electronAPI.appWindowClose) {
            window.electronAPI.appWindowClose('web-window-1');
            return;
        }
...
function setupWalletButtons() {
    ...
    document.getElementById('copyWalletAddressBtn')?.addEventListener('click', async () => {
        ...
        if (network === 'solana') {
            ...
            networkName = network === 'omega' ? 'AVAX (Fuji)' : network === 'bsc' ? 'BSC' : 'Ethereum';
        }
    ...
    document.getElementById('sendChain')?.addEventListener('change', (e) => {
        ...
        if (chain === 'solana') {
            ...
            addressInput.placeholder = 'Enter Avalanche Fuji address';
    ...
    document.getElementById('sendTransactionBtn')?.addEventListener('click', async () => {
        ...
        if (chain === 'solana') {
            ...
            chainName = 'OMEGA';
            chainId = 43113;  // Avalanche Fuji C-Chain (OS default)
        } else if (chain === 'bsc') {
            chainName = 'BNB';
            chainId = 56;
        } else {
...
async function refreshWalletBalance(networkOverride = null) {
    ...
    try {
        if (finalNetwork === 'solana') {
            ...
            // Avalanche Fuji C-Chain (OS default)
            try {
                const omegaBalance = await window.electronAPI.walletGetEvmBalance(43113);
                const balanceNum = parseFloat(omegaBalance);
                if (isNaN(balanceNum)) {
                    throw new Error('Invalid balance response');
                }
                const balanceText = balanceNum.toFixed(4) + ' AVAX';
                balanceAmount.textContent = balanceText;
...
function updateActivityList() {
    ...
    activityList.innerHTML = activities.map(activity => {
        ...
        if (activity.hash) {
            ...
            if (network === 'omega') {
                explorerUrl = `https://testnet.snowtrace.io/tx/${activity.hash}`;
