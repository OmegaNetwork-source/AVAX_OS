...
function createAppWindow(appType, options = {}) {
  ...
  if (appType === 'browser') {
    ...
    width = options.width || 1000;
    height = options.height || 700;
  } else if (appType === 'dappfun') {
    // Local dev server (e.g. npm run dev in dapp.fun → Vite on 5173)
    appFile = 'webapp-wrapper.html';
    width = options.width || 1000;
    height = options.height || 700;
    loadUrl = process.env.DAPPFUN_URL || 'http://localhost:5173/';
  } else if (appType === 'avax-arcade') {
    // Local dev server (e.g. npx serve in AVAX Arcade → port 3000)
    appFile = 'webapp-wrapper.html';
    width = options.width || 1000;
    height = options.height || 700;
    loadUrl = process.env.AVAX_ARCADE_URL || 'http://localhost:62678/AVAX%20Arcade/avax-arcade.html';
  } else {
    return null;
  }
    ...
    try {
      ...
      switch (request.method) {
        ...
        case 'eth_sendTransaction':
          ...
          // Notify wallet window of app interaction
          const chainName = txChainId === 43113 ? 'omega' : txChainId === 56 ? 'bsc' : 'evm';
          notifyWalletAppInteraction('app_interaction', {
            type: 'send',
            network: chainName,
            ...
            to: tx.to,
            amount: valueEth,
            symbol: chainName === 'omega' ? 'AVAX' : chainName === 'bsc' ? 'BNB' : 'ETH'
          });
          break;
