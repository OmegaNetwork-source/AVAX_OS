    ...
    desktopIcons.forEach(icon => {
        ...
        document.addEventListener('mouseup', () => {
            if (isDragging) {
                ...
                // Snap to grid and save when drag ends
                const rect = icon.getBoundingClientRect();
                const snapped = snapToGrid(rect.left, rect.top);
                icon.style.left = snapped.x + 'px';
                icon.style.top = snapped.y + 'px';
                saveIconPosition(icon.id, snapped.x, snapped.y);
            }
        });
...
// Strict desktop grid so icons align side-by-side and up-down like a normal desktop
const DESKTOP_GRID = {
    cols: 4,
    cellWidth: 96,
    cellHeight: 118,
...
function snapToGrid(x, y) {
    ...
    const col = Math.round((x - g.startX) / g.cellWidth);
    const row = Math.round((y - g.startY) / g.cellHeight);
    const snappedX = g.startX + Math.max(0, col) * g.cellWidth;
    const snappedY = g.startY + Math.max(0, row) * g.cellHeight;
...
function getGridPosition(index) {
    ...
    return {
        x: g.startX + col * g.cellWidth,
        y: g.startY + row * g.cellHeight
    };
}
...
function loadIconPositions() {
    ...
    icons.forEach((icon, index) => {
        ...
        if (savedPositions[iconId]) {
            const snapped = snapToGrid(savedPositions[iconId].x, savedPositions[iconId].y);
            x = snapped.x;
            y = snapped.y;
        } else {
...
const DEFAULT_CRYPTO_SELECTION = ['avalanche-2', 'bitcoin', 'ethereum'];

// Map our coin ids to Binance USDT symbols (Binance API allows CORS for public data)
...
// Map our coin ids (CoinGecko-style) to CoinCap API asset ids
const COIN_ID_TO_COINCAP = {
...
// Fetch URL; in web mode try CORS proxies. Uses response.text() then parse (proxies often return text).
async function fetchJsonWithCors(url, isWeb, signal) {
    const proxyUrls = [
        'https://api.allorigins.win/raw?url=' + encodeURIComponent(url),
        'https://corsproxy.io/?' + encodeURIComponent(url)
    ];
    const urls = isWeb ? proxyUrls : [url];
    let lastErr;
    for (let i = 0; i < urls.length; i++) {
        try {
            const res = await fetch(urls[i], { method: 'GET', headers: { 'Accept': 'application/json' }, signal: signal });
            ...
            try {
                ...
                var wrapper = null;
                try { wrapper = JSON.parse(text); } catch (e2) {}
...
// Apply price data (from CoinGecko or CoinCap) to DOM and cache. data: { [coinId]: { usd, usd_24h_change } }
function applyCryptoPriceData(selectedIds, data, cached) {
    let hasAny = false;
    selectedIds.forEach(coinId => {
        const coinData = data[coinId];
        if (!coinData || coinData.usd == null) return;
        hasAny = true;
...
// Fetch prices from Binance (direct, no proxy - Binance allows CORS for public ticker). Returns { [coinId]: { usd, usd_24h_change } } or null.
async function fetchBinancePrices(selectedIds, signal) {
    const symbols = selectedIds.map(function(id) { return COIN_ID_TO_BINANCE[id]; }).filter(Boolean);
    if (symbols.length === 0) return null;
    const url = 'https://api.binance.com/api/v3/ticker/24hr?symbols=' + encodeURIComponent(JSON.stringify(symbols));
    try {
        const res = await fetch(url, { method: 'GET', signal: signal || null });
        if (!res.ok) return null;
        const arr = await res.json();
        if (!Array.isArray(arr)) return null;
        const symbolToId = {};
        selectedIds.forEach(function(id) {
            var s = COIN_ID_TO_BINANCE[id];
            if (s) symbolToId[s] = id;
        });
        const out = {};
        arr.forEach(function(item) {
            var id = symbolToId[item.symbol];
            if (!id || item.lastPrice == null) return;
            var price = parseFloat(item.lastPrice);
            var change = item.priceChangePercent != null ? parseFloat(item.priceChangePercent) : 0;
            if (!isNaN(price)) out[id] = { usd: price, usd_24h_change: change };
        });
...
// Fetch prices from CoinCap. Returns { [coinId]: { usd, usd_24h_change } } or null.
async function fetchCoinCapPrices(selectedIds, isWeb, signal) {
    const results = {};
    await Promise.all(selectedIds.map(async (coinId) => {
        const coincapId = COIN_ID_TO_COINCAP[coinId] || coinId;
        const url = 'https://api.coincap.io/v2/assets/' + coincapId;
        try {
            const json = await fetchJsonWithCors(url, isWeb, signal);
            const d = json && json.data;
            if (d && d.priceUsd != null) {
                const change = d.changePercent24Hr != null ? parseFloat(d.changePercent24Hr) : 0;
                results[coinId] = { usd: parseFloat(d.priceUsd), usd_24h_change: change };
            }
        } catch (e) {
            console.warn('CoinCap fetch failed for', coinId, e.message);
        }
    }));
    return Object.keys(results).length > 0 ? results : null;
}

// Crypto Price Widget - real prices only: CoinGecko first, then CoinCap; no simulation
async function updateCryptoPrices() {
    const selectedIds = loadSelectedCryptos();
    if (selectedIds.length === 0) return;

    const isWeb = typeof window !== 'undefined' && (!window.electronAPI || window.OMEGA_WEB_MODE);
    ...
    if (cached) {
        try {
            ...
            if (Date.now() - (cachedData.timestamp || 0) < 120000) {
                selectedIds.forEach(coinId => {
                    ...
                    if (coinData) {
                        ...
                        if (changeEl) { changeEl.textContent = coinData.change; changeEl.className = coinData.changeClass || ''; }
                    }
                });
    ...
    // 1) Try Binance first (direct fetch - no proxy; Binance allows CORS for public ticker)
    var data = null;
    try {
        data = await fetchBinancePrices(selectedIds, controller.signal);
    } catch (e) {
        console.warn('Binance failed:', e.message);
    }

    // 2) Try CoinGecko (direct in Electron, via CORS proxies in web)
    if (!data) {
        var geckoUrl = 'https://api.coingecko.com/api/v3/simple/price?ids=' + encodeURIComponent(selectedIds.join(',')) + '&vs_currencies=usd&include_24hr_change=true';
        if (!isWeb) {
            try {
                var res = await fetch(geckoUrl, { method: 'GET', signal: controller.signal });
                if (res.ok) {
                    var json = await res.json();
                    if (json && typeof json === 'object') {
                        var normalized = {};
                        selectedIds.forEach(function(coinId) {
                            var c = json[coinId];
                            if (c && typeof c.usd === 'number') normalized[coinId] = { usd: c.usd, usd_24h_change: (c.usd_24h_change != null ? c.usd_24h_change : 0) };
                        });
                        if (Object.keys(normalized).length > 0) data = normalized;
                    }
                }
            } catch (e) {
                console.warn('CoinGecko direct failed:', e.message);
            }
        }
        if (!data) {
            for (var attempt = 0; attempt < 3; attempt++) {
                try {
                    var json = await fetchJsonWithCors(geckoUrl, isWeb, controller.signal);
                    if (json && typeof json === 'object') {
                        var normalized = {};
                        selectedIds.forEach(function(coinId) {
                            var c = json[coinId];
                        ...
                        if (Object.keys(normalized).length > 0) { data = normalized; break; }
                    }
                } catch (e) {
                    console.warn('CoinGecko attempt', attempt + 1, e.message);
    ...
    // 3) Try CoinCap (direct in Electron, via proxies in web)
    if (!data) {
        try {
            data = await fetchCoinCapPrices(selectedIds, isWeb, controller.signal);
        } catch (e) {
    ...
    if (data && applyCryptoPriceData(selectedIds, data)) return;

    // All sources failed - show N/A; retry once after 3s (in case of slow network)
    selectedIds.forEach(function(coinId) {
        var priceEl = document.getElementById(coinId + 'Price');
        var changeEl = document.getElementById(coinId + 'Change');
        if (priceEl) priceEl.textContent = '—';
        if (changeEl) { changeEl.textContent = '—'; changeEl.className = 'crypto-change neutral'; }
    });
    console.warn('Crypto prices unavailable: Binance, CoinGecko and CoinCap failed');
    if (isWeb && !window._cryptoPriceRetried) {
        window._cryptoPriceRetried = true;
        setTimeout(function() { updateCryptoPrices(); }, 3000);
    }
}
...
- Crypto Price Widget: Real-time AVAX, BTC, ETH prices in taskbar (customizable)
...
function getLocalResponse(message) {
    ...
    const responses = {
        ...
        'crypto': 'Crypto prices (AVAX, BTC, ETH) are shown in the taskbar. Click the widget to customize which coins to display.',
...
function setupIdentityRegistration() {
    ...
    // Only override "Register Here" in real Electron (desktop app). In browser, let the link navigate to /identity in same tab so MetaMask works.
    var isRealElectron = window.electronAPI && typeof window.electronAPI.identityInitialize === 'function';
    if (userRegisterLink && isRealElectron) {
