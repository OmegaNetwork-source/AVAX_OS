 ...
 * Manages decentralized OS identity on Avalanche Fuji Testnet (C-Chain)
 */
class OmegaIdentityManager {
    constructor() {
        // Avalanche Fuji Testnet (C-Chain) - override with env if needed
        this.omegaNetworkRpc = process.env.OMEGA_NETWORK_RPC || process.env.AVAX_RPC || 'https://api.avax-test.network/ext/bc/C/rpc';
        this.omegaNetworkChainId = parseInt(process.env.OMEGA_NETWORK_CHAIN_ID || process.env.AVAX_CHAIN_ID || '43113'); // Avalanche Fuji
        
        // Paste your Remix-deployed contract addresses (Avalanche Fuji) here, or set env vars:
        // OMEGA_IDENTITY_CONTRACT, OMEGA_LICENSING_CONTRACT, OMEGA_SYNC_CONTRACT
        this.identityContractAddress = process.env.OMEGA_IDENTITY_CONTRACT || '0x93932a1608910f50Ac2bC8A9B8A7183010a93e01';
        this.licensingContractAddress = process.env.OMEGA_LICENSING_CONTRACT || '0x67DDb9eDAF5CBc94e6ab08988FaF1b8CE16256dc';
        this.syncContractAddress = process.env.OMEGA_SYNC_CONTRACT || '0x41368Ab62ad2DA4D42C95e0fed16bDf0FbaF4778';
    ...
    async createOrLoadIdentity(evmWallet) {
        try {
            ...
            // Register identity on Avalanche Fuji
            const registered = await this.registerIdentityOnChain(omegaId, address, deviceFingerprint);
            
            if (!registered) {
                throw new Error('Failed to register identity on Avalanche Fuji');
            }
    ...
    /**
     * Register identity on Avalanche Fuji (C-Chain)
     */
    async registerIdentityOnChain(omegaId, address, deviceFingerprint) {
        try {
            ...
            // Register identity
            console.log('Registering identity on Avalanche Fuji...', { omegaId, address, deviceFingerprint });
    ...
    /**
     * Sync document hash to Avalanche Fuji
     */
    async syncDocumentHash(documentId, documentHash, metadata = {}) {
        try {
            ...
            console.log('Syncing document to Avalanche Fuji...', { 
